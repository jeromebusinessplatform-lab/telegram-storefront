...
const statusConfig = {
  ...
  processing: { label: 'Processing', icon: Clock, color: 'text-primary', bg: 'bg-primary/10' },
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-2 mb-3">
        {preview.map((item) => (
          <div key={item.product.id} className="w-14 h-14 rounded-xl overflow-hidden bg-muted relative flex-shrink-0">
            <img src={item.product.image} alt={item.product.name} className="w-full h-full object-cover" />
            {item.quantity > 1 && (
              <span className="absolute bottom-0.5 right-0.5 bg-foreground/80 text-background text-[9px] font-bold rounded-md px-1">
                ×{item.quantity}
              </span>
            )}
          </div>
        ...
        {order.items.length > 3 && (
          <div className="w-14 h-14 rounded-xl bg-muted flex items-center justify-center text-xs font-bold text-muted-foreground">
            +{order.items.length - 3}
          </div>
        )}
      </div>
...
export default function OrdersPage() {
  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      {orders.length === 0 ? (
        <div className="flex flex-col items-center justify-center flex-1 gap-4 px-8 text-center pb-16">
