...
import { Search, Plus, Pencil, Trash2, AlertTriangle, Package, Tag } from 'lucide-react';
...
export default function InventoryPage() {
  ...
  const handleEdit  = (p: Product) => { setEditProduct(p); setDrawerOpen(true); };
  const handleAdd   = ()           => { setEditProduct(undefined); setDrawerOpen(true); };
  const handleDelete = (id: string) => { deleteProduct(id); setConfirmDelete(null); toast.success('Product deleted'); };

  const handleAddCategory = (name: string) => { addCategory(name); };
              ...
              <motion.div key={p.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    ...
                    {p.bundle?.enabled && <Tag size={9} className="text-primary flex-shrink-0" />}
                  </div>
                  <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                    ...
                    {p.subName && (
                      <span className="text-[9px] text-muted-foreground/70 italic line-clamp-1">{p.subName}</span>
                    )}
                    <span className="text-[10px] font-bold text-foreground">₱{fmtPrice(p.price)}</span>
                    {p.badge && (
                ...
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  <button onClick={() => handleEdit(p)} className="w-7 h-7 rounded-lg bg-primary/10 text-primary flex items-center justify-center">
                    <Pencil size={12} />
                  </button>
                  <button onClick={() => setConfirmDelete(p.id)} className="w-7 h-7 rounded-lg bg-destructive/10 text-destructive flex items-center justify-center">
                    <Trash2 size={12} />
                  </button>
                </div>
              </motion.div>
