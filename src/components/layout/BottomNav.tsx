...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-card/96 backdrop-blur-md border-t border-border max-w-sm mx-auto">
      <div className="flex items-center justify-around px-0 py-1.5">
        {navItems.map(({ to, label, icon: Icon, exact, badge }) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `relative flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-xl transition-all duration-200 ${
                isActive
                  ? 'text-primary'
              ...
              <>
                  ...
                  <AnimatePresence>
                    {badge === 'cart' && totalItems > 0 && (
                      <motion.span
                        ...
                        animate={{ scale: 1 }}
                        exit={{ scale: 0 }}
                        className="absolute -top-1.5 -right-1.5 min-w-[14px] h-3.5 px-1 bg-primary text-primary-foreground text-[8px] font-bold rounded-full flex items-center justify-center leading-none"
                      >
                ...
                {isActive && (
                  <motion.div
                    ...
                    className="absolute -bottom-0 w-4 h-0.5 rounded-full bg-primary"
                  />
                )}
              </>
            )}
