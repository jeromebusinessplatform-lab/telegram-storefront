...
import { X, Plus, Trash2, Upload, Link, ChevronDown, Package } from 'lucide-react';
...
export default function ProductFormDrawer({ open, onClose, product, categories, allProducts, onSave }: Props) {
          ...
          <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div>
                    ...
                    {imageMode === 'url' ? (
                      <input value={image} onChange={e => setImage(e.target.value)} placeholder="https://..."
                        className="w-full bg-muted rounded-xl px-3 py-2.5 text-[11px] text-foreground outline-none focus:ring-1 focus:ring-primary/30 border border-border/40" />
                    ) : (
                      <>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <select value={category} onChange={e => setCategory(e.target.value)}
                        className="w-full appearance-none bg-muted rounded-xl px-3 py-2.5 text-[11px] text-foreground outline-none border border-border/40 pr-8">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div>
                      ...
                      <input type="number" value={costing} onChange={e => setCosting(e.target.value)} placeholder="0.00" min="0"
                        className="w-full bg-muted rounded-xl px-3 py-2.5 text-[11px] text-foreground outline-none border border-border/40 focus:ring-1 focus:ring-primary/30" />
                    </div>
                  </div>
                    ...
                    <div key={v.id} className="bg-muted rounded-2xl p-3 border border-border/40">
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <div className="grid grid-cols-3 gap-1.5">
                          <div>
                            <p className="text-[9px] text-muted-foreground mb-1">Price ₱</p>
                            <input type="number" value={v.price} onChange={e => updateVariant(v.id, 'price', Number(e.target.value))} min="0"
                              className="w-full bg-card rounded-lg px-2 py-1.5 text-[11px] text-foreground outline-none border border-border/40" />
                          </div>
                          <div>
                            <p className="text-[9px] text-muted-foreground mb-1">Costing ₱</p>
                            <input type="number" value={v.costing} onChange={e => updateVariant(v.id, 'costing', Number(e.target.value))} min="0"
                              className="w-full bg-card rounded-lg px-2 py-1.5 text-[11px] text-foreground outline-none border border-border/40" />
                          </div>
                          <div>
                            <p className="text-[9px] text-muted-foreground mb-1">Stock</p>
                            <input type="number" value={v.stock} onChange={e => updateVariant(v.id, 'stock', Number(e.target.value))} min="0"
                              className="w-full bg-card rounded-lg px-2 py-1.5 text-[11px] text-foreground outline-none border border-border/40" />
                          </div>
                        </div>
                      </div>
              ...
              {tab === 'Promotions' && (
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex items-center justify-between bg-muted rounded-2xl px-4 py-3 border border-border/40">
                    ...
                    <button onClick={() => setBundle(b => ({ ...b, enabled: !b.enabled }))}
                      className={`w-10 h-5.5 rounded-full relative transition-colors ${bundle.enabled ? 'bg-primary' : 'bg-border'}`}
                      style={{ minWidth: 40, height: 22 }}>
                      <motion.div animate={{ x: bundle.enabled ? 18 : 2 }} transition={{ type: 'spring', stiffness: 500, damping: 30 }}
                    ...
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}
                      ...
                      <div className="bg-card rounded-2xl border border-border/40 p-3">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <button onClick={() => setBundle(b => ({ ...b, useGlobalPrice: !b.useGlobalPrice }))}
                            className={`rounded-full relative transition-colors flex-shrink-0`}
                            style={{ minWidth: 36, height: 20, background: bundle.useGlobalPrice ? 'hsl(var(--primary))' : 'hsl(var(--border))' }}>
                            <motion.div animate={{ x: bundle.useGlobalPrice ? 16 : 2 }} transition={{ type: 'spring', stiffness: 500, damping: 30 }}
                              className="absolute top-0.5 w-4 h-4 rounded-full bg-white shadow-sm" />
                          </button>
                        </div>
                        {bundle.useGlobalPrice && (
                          <div>
                            <label className="text-[10px] text-muted-foreground mb-1 block">Bundle Total Price ₱</label>
                            <input type="number" value={bundle.globalPrice ?? ''} onChange={e => setBundle(b => ({ ...b, globalPrice: Number(e.target.value) }))}
                              placeholder="0.00" min="0"
                              className="w-full bg-muted rounded-xl px-3 py-2 text-[11px] text-foreground outline-none border border-border/40" />
                          </div>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {bundle.items.length === 0 && (
                          <div className="text-center py-5 text-[11px] text-muted-foreground">No products added yet</div>
                        )}
                            ...
                            <div key={item.id} className="bg-card rounded-2xl border border-border/40 p-3 mb-2">
                              ...
                              <div className="relative mb-2">
                                <select value={item.productId} onChange={e => updateBundleItem(item.id, 'productId', e.target.value)}
                                  className="w-full appearance-none bg-muted rounded-xl px-3 py-2 text-[11px] text-foreground outline-none border border-border/40 pr-8">
                                    ...
                                    {selectedProduct && (
                                      <span className="text-[10px] text-muted-foreground whitespace-nowrap">
                                        → ₱{item.discountType === 'fixed'
                                          ...
                                          : (selectedProduct.price * (1 - item.discountValue / 100)).toFixed(0)}
                                      </span>
                                    )}
                                  </div>
                                </div>
            ...
            <div className="px-5 pb-6 pt-3 flex-shrink-0 border-t border-border/40">
              <button onClick={handleSave}
                className="w-full py-3.5 bg-primary text-primary-foreground rounded-2xl text-sm font-semibold shadow-elevated">
